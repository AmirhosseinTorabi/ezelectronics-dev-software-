...
class ProductRoutes {
  ...
  initRoutes() {
    ...
    this.router.post(
      ...
      body("arrivalDate", "Invalid Arrival Data").optional().isDate({
        format: "YYYY-MM-DD",
      }),
    ...
    this.router.patch(
      ...
      body("quantity", "Invalid quantity")
        ...
        .notEmpty()
        .isInt({ gt: 0 }),
      body("changeDate", "Invalid changeDate").optional().isDate({ format: "YYYY-MM-DD" }),
      (req: any, res: any, next: any) => this.errorHandler.validateRequest(req, res, next),
      (req: any, res: any, next: any) => {
        if (req.body.changeDate === undefined) {
    ...
    this.router.patch(
      ...
      body("quantity", "Invalid quantity")
        ...
        .notEmpty()
        .isInt({ gt: 0 }),
      body("sellingDate", "Invalid sellingDate").optional().isDate({
        format: "YYYY-MM-DD",
      }),
      (req: any, res: any, next: any) => this.errorHandler.validateRequest(req, res, next),
      (req: any, res: any, next: any) => {
        if (req.body.sellingDate === undefined) {
            req.body.sellingDate = null;
        }
        next();
      },
      (req: any, res: any, next: any) =>
        this.controller
          ...
          .then((quantity: any) => res.status(200).json({ quantity: quantity }))
          .catch((err) => next(err))
    );
    ...
    this.router.get(
      ...
      (req: any, res: any, next: any) => this.errorHandler.validateRequest(req, res, next),
      (req: any, res: any, next: any) => {
        if (req.query.grouping === undefined) {
          req.query.grouping = null;
        }
