...
class ReviewRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            param("model", "model must not be empty").isString().notEmpty(), 
            body("comment", "comment must not be null").exists().isString().notEmpty(), 
            body("score", "score must between 1 and 5").isNumeric().isInt({ min:1, max:5}), 
